...
public class Login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (schoolList.size() == 0) {
			...
			// User newUser = new User(firstName, familyName, password,
			// isTeacher, null, schoolList.get(0).getKey());

			String queryString = "SELECT u FROM User u WHERE firstName='"
					+ firstName + "' AND familyName='" + familyName
					+ "' AND password='" + password + "' AND isTeacher="
					+ isTeacher + " AND school='" + schoolList.get(0).getKey()
					+ "'";
